...
if reverseFilter
    ...
    for idx = 1 : nSections
        y1 = out - states{idx}.z1_A - states{idx}.z2_A;
        out = coeff{idx}.b0(1) .* y1 + coeff{idx}.d1(1) .* states{idx}.z1_A + coeff{idx}.d2(1) .* states{idx}.z2_A;
    end
    tmp2(m, :) = out;
    tmp2 = flipud(tmp2);
else
...
sub = tmp2 * dp - target; % MSE
mse1 = sum(sub.^2); % MSE
grad1 = mse1.backprop(1); % Backpropation, aka, training
toc
