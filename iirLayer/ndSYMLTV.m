    ...
    if reverseFilter
        for a = m : -1 : 2
            ...
            for idx = 1 : nSections
                ...
                out(idx, :) = b0(a, idx) .* y1 + d1(a, idx) .* states{idx}.z1_A + d2(a, idx) .* states{idx}.z2_A;
                states{idx}.z2_A = states{idx}.z2_A + c2(a, idx) .* states{idx}.z1_A;
                states{idx}.z1_A = states{idx}.z1_A + c1(a, idx) .* y1;
            end
            tmp2(a, :) = sum(out, 1);
        end
        in = input(1, :);
        for idx = 1 : nSections
            y1 = in - states{idx}.z1_A - states{idx}.z2_A;
            out(idx, :) = b0(1, idx) .* y1 + d1(1, idx) .* states{idx}.z1_A + d2(1, idx) .* states{idx}.z2_A;
        end
        tmp2(1, :) = sum(out, 1);
    else
        for a = 1 : m - 1
        ...
        for idx = 1 : nSections
            y1 = in - states{idx}.z1_A - states{idx}.z2_A;
            out(idx, :) = b0(m, idx) .* y1 + d1(m, idx) .* states{idx}.z1_A + d2(m, idx) .* states{idx}.z2_A;
        end
        tmp2(m, :) = sum(out, 1);
    end
else
    if reverseFilter
        for a = m : -1 : 2
            ...
            for idx = 1 : nSections
                ...
                out = b0(a, idx) .* y1 + d1(a, idx) .* states{idx}.z1_A + d2(a, idx) .* states{idx}.z2_A;
                states{idx}.z2_A = states{idx}.z2_A + c2(a, idx) .* states{idx}.z1_A;
                states{idx}.z1_A = states{idx}.z1_A + c1(a, idx) .* y1;
            end
            tmp2(a, :) = out;
        ...
        for idx = 1 : nSections
            y1 = out - states{idx}.z1_A - states{idx}.z2_A;
            out = b0(1, idx) .* y1 + d1(1, idx) .* states{idx}.z1_A + d2(1, idx) .* states{idx}.z2_A;
        end
        tmp2(1, :) = out;
    else
        for a = 1 : m - 1
        ...
        for idx = 1 : nSections
            y1 = out - states{idx}.z1_A - states{idx}.z2_A;
            out = b0(m, idx) .* y1 + d1(m, idx) .* states{idx}.z1_A + d2(m, idx) .* states{idx}.z2_A;
        end
        tmp2(m, :) = out;
    end
end
