function [coeff, f] = precomuteLS_Blank(fftLen, hop, fs, oct, order, reqSynthesisWnd, sparCutOff, zp)
rng(1)
addpath('../')
...
if dbg2
    ...
    for i = 1 : ovp - 1
        stepSize1 = mod(fftLen - hop * i, fftLen);
        stepSize = mod(fftLen - hop * i - fftLen / 2 - 1, fftLen);
        phaseShifter = analysisWnd(stepSize1) * exp(-1i * stepSize * (2*pi*(0:fftLen-1)/fftLen)');
        q_fft_frameinv = ltv_1st_2ndNoSq2(phaseShifter, b, a, c1, c2);
        lastElement = q_fft_frameinv(end);
        q_fft_frameinv = q_fft_frameinv(1 : halfLen);
        ...
        q_fft_frameinv2 = shtfft .* q_fft_frameinv(1 : end);
        ...
        tmp(:, i) = ifft(q_fft_frameinv2);
    end
    systemImpulse = overlapAdd(tmp, hop);
    unityG = abs(fft(systemImpulse));
    % unityG = unityG(1 : getFFTHalfLen(size(unityG, 1)));
    unityG = 1 ./ interp1(linspace(0, 1, length(unityG)), unityG, linspace(0, 1, fftLen))';
    % unityG(:)=1;
    clear cpxRes dftFilterbank2 theoreticalWindowShape2 theoreticalWindowShape
    windowedDFTMatrix1 = dftMtx .* actualWindowShape3;
    LSsolution1 = ifft(windowedDFTMatrix1.') .* unityG;
    ...
    if enableHFCorr
        if order == 2
            wndCorrectionWeighting = max(b) - b;
        else
            wndCorrectionWeighting = max(a) - a;
        end
        ...
        wndCorrectionWeighting(halfLen+1:fftLen) = conj(wndCorrectionWeighting(halfLen-1:-1:2));
        LSsolution2 = (Phi0 / dftMtx);
        windowedDFTMatrix2 = fft(LSsolution2).';
        actualWindowShape4 = conj(dftMtx) .* windowedDFTMatrix2;
        hfWnd = actualWindowShape4(halfLen, :);
        correctionWndHF_ = overlapAdd(repmat(hfWnd, ovp * 2, 1 )', hop);
        correctionWndHF2 = correctionWndHF_(paddedFFTLen - eachSide + 1 : paddedFFTLen - eachSide + fftLen);
        correctionWndHF = real(1 ./ correctionWndHF2(:));
        % sigVec = randn(fftLen, 1);
        LSsolution5 = ifft(dftMtx .* correctionWndHF);
        LSsolution3 = LSsolution2 .* (1 - wndCorrectionWeighting) + (LSsolution5 * LSsolution2) .* wndCorrectionWeighting;
        Phi2 = LSsolution3 * dftMtx;
        Phi2 = Phi2(1 : halfLen, :);
        Phi2(1, :) = real(Phi2(1, :));
        Phi2(end, :) = real(Phi2(end, :));
        Phi2(halfLen+1:fftLen, :) = conj(Phi2(halfLen-1:-1:2, :));
        Phi0 = Phi2;
    else
        correctionWndHF = [];
    ...
    % Xa = fft(fftshift(sigVec .* analysisWnd)); % compute analysis filter outputs of a block of L samples
    % q_fft_frameinv = ltv_1st_2ndNoSq2(Xa, b, a, c1, c2);
    % lastElement = q_fft_frameinv(end);
    % q_fft_frameinv = q_fft_frameinv(1 : halfLen);
    % q_fft_frameinv = 0.5 * (q_fft_frameinv + 0.5 * [lastElement; q_fft_frameinv(1 : end - 1)] + 0.5 * [q_fft_frameinv(2 : end); conj(q_fft_frameinv(end - 1))]);
    % q_fft_frameinv2 = shtfft2 .* q_fft_frameinv(1 : end);
    % q_fft_frameinv2(1) = real(q_fft_frameinv2(1));
    % q_fft_frameinv2(end) = real(q_fft_frameinv2(end));
    % q_fft_frameinv2(halfLen+1:fftLen) = conj(q_fft_frameinv2(halfLen-1:-1:2));
    % td2 = ifft(q_fft_frameinv2);
    % tdCorrHF2 = td2 .* correctionWndHF;
    % fd2 = q_fft_frameinv2 .* (1 - wndCorrectionWeighting) + fft(tdCorrHF2) .* wndCorrectionWeighting;
    %%
    % plot(abs(fd3));
    % hold on
    % plot(abs(fd2));
...
coeff.correctionWndHF = correctionWndHF;
outterPad = 0;
%%
...
for m = 1 : nframes % index of window position
    ...
    q_fft_frameinv = ltv_1st_2ndNoSq2(Xa, b, a, c1, c2);
    lastElement = q_fft_frameinv(end);
    q_fft_frameinv = q_fft_frameinv(1 : origHalfLen);
    q_fft_frameinv = 0.5 * (q_fft_frameinv + 0.5 * [lastElement; q_fft_frameinv(1 : end - 1)] + 0.5 * [q_fft_frameinv(2 : end); conj(q_fft_frameinv(end - 1))]);
    q_fft_frameinv2 = shtfft2 .* q_fft_frameinv(1 : end) .* unityG(1 : origHalfLen);
    q_fft_frameinv2(1) = real(q_fft_frameinv2(1));
    q_fft_frameinv2(end) = real(q_fft_frameinv2(end));
    q_fft_frameinv2(origHalfLen+1:origFFTLen) = conj(q_fft_frameinv2(origHalfLen-1:-1:2));
    if ~isempty(correctionWndHF)
        td2 = ifft(q_fft_frameinv2);
        tdCorrHF2 = td2 .* correctionWndHF;
        fd2 = q_fft_frameinv2 .* (1 - wndCorrectionWeighting) + fft(tdCorrHF2) .* wndCorrectionWeighting;
    end
...
for i=1:nframes
    ...
    lastElement = q_fft_frameinv(end);
    q_fft_frameinv = q_fft_frameinv(1 : coeff.origHalfLen);
    q_fft_frameinv = 0.5 * (q_fft_frameinv + 0.5 * [lastElement; q_fft_frameinv(1 : end - 1)] + 0.5 * [q_fft_frameinv(2 : end); conj(q_fft_frameinv(end - 1))]);
    q_fft_frameinv2 = shtfft2 .* q_fft_frameinv .* coeff.unityG(1 : coeff.origHalfLen);
    q_fft_frameinv2(1) = real(q_fft_frameinv2(1));
    q_fft_frameinv2(end) = real(q_fft_frameinv2(end));
    if ~isempty(coeff.correctionWndHF)
        q_fft_frameinv2(coeff.origHalfLen+1:coeff.origFFTLen) = conj(q_fft_frameinv2(coeff.origHalfLen-1:-1:2));
        td2 = ifft(q_fft_frameinv2);
        tdCorrHF2 = td2 .* coeff.correctionWndHF;
        fd2 = q_fft_frameinv2 .* (1 - coeff.wndCorrectionWeighting) + fft(tdCorrHF2) .* coeff.wndCorrectionWeighting;
        S(i, :) = fd2(1 : coeff.origHalfLen);
    else
        S(i, :) = q_fft_frameinv2;
    end
end
