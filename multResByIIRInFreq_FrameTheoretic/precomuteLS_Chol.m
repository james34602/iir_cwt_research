...
%%
% R2_ = chol(C2, 'upper');
[R2_, R2Sp] = chol_sec_nr_blockShortCut3(C3, solCholSize, fftLen, hop);
%% Cholesky
cuttedPos = tPhiSel(fftLen+1:ck*fftLen, :);
cc = [zeros(padUp - fftLen, halfLen); tPhi0; zeros((ck*fftLen-fftLen)-padUp, halfLen)];
validRng = [padUp - fftLen + 1, padUp - fftLen + fftLen];
...
% cuttedPos2(:)=randn(size(cuttedPos2));
Phi_s4 = R2_ \ (R2_' \ cuttedPos2); % pseudo inverse
mine2 = solveTri(R2_', cuttedPos2, R2Sp);
...
for j = 1 : size(b1, 2)
    ...
    for k=2:n
        ary = L(k, k - 1 : -1 : 1);
        mtx1(1 : length(ary), k) = ary;
        % if k > 33
        %     plot(L(k, k - 1 : -1 : 1));axis tight
        %     disp('')
        % end
        x1 = 1 ./ L(k, k) .* (b1(k, j) - sum(L(k, k - 1 : -1 : 1) .* b(k - 1 : -1 : 1).'));
        b(k) = x1;
    end
    mtx2 = zeros(S);
    mtx2(k, k) = L(n, n);
    g(n, j) = b(end) ./ L(n, n);
    for k = n - 1 : -1 : 1
        % plot(L(k + 1 : end, k));axis tight
        ary = L(k + 1 : end, k);
        mtx2(1 : length(ary), k) = ary;
        x1 = 1 ./ L(k, k) .* (b(k) - sum(L(k + 1 : end, k) .* g(k + 1 : end, j)));
        g(k, j) = x1;
    end
end
end
function [U, U3] = chol_sec_nr_blockShortCut3(submatrix, solCholSize, fftLen, hop)
U = zeros(solCholSize, solCholSize);
U3 = zeros(fftLen, solCholSize);
% U3 = zeros(solCholSize, solCholSize);
inds = 1:hop;
...
for n = 1 : solCholSize / hop
    ...
    % Utr = linsolve(Utl', selectedA, opts);
    Utr = Utl' \ selectedA;
    Utl2 = Utl';
    Utr2 = Utr';
    ...
    % for i = 1 : hop
    %     U2(i + myOffset, 1 : (hop - i + 1)) = Utl(i, i : end);
    %     U2(myOffset + i, (hop - i + 1) + 1 : (hop - i + 1) + size(Utr, 2)) = Utr(i, :);
    % end
    for i = 1 : hop
        U3(1 : (hop - i + 1), myOffset + i) = Utl2(i : end, i);
        U3((hop - i + 1) + 1 : (hop - i + 1) + size(Utr, 2), myOffset + i) = Utr2(:, i);
    end
    % for i = 1 : hop
    %     U3(myOffset + i + (0 : (hop - i)), myOffset + i) = Utl2(i : end, i);
    %     U3(myOffset + hop + 1 : myOffset + hop + size(Utr, 2), myOffset + i) = Utr2(:, i);
    % end
    gram = Utr' * Utr;
    % imagesc(log(abs(U)))
    % clim([-3, 5.2])
...
for n = 1 : fftLen
    inc = inc + 1;
    U4(n, offset + 1 : inc + 1) = U2(n, offset + 1 : inc + 1);
end
offset = hop;
remaining = solCholSize - fftLen;
for n = (fftLen + 1) : hop : solCholSize
    for i = 0 : min(hop - 1, remaining - 1)
        U4(n + i, 1 : (inc + i + 1) - (offset)) = U2(n + i, offset + 1 : inc + i + 1);
    end
    offset = offset + hop;
    inc = inc + hop;
    remaining = remaining - hop;
