...
% windowedDFTMatrix3 = fft(LSsolution2).';
% kk = conj(dftMtx) .* windowedDFTMatrix3;
origHalfLen = halfLen;
origFFTLen = fftLen;
% halfLen = fftLen;
...
sigLen = 32768;
x = randn(sigLen, 1);
x = [zeros(size(Phi_s2, 1) * 1, 1); x(:); zeros(size(Phi_s2, 1), 1)];
