function [coeff, f] = precomuteLSIIR(fftLen, hop, fs, oct, order, reqSynthesisWnd, sparCutOff, zp)
rng(1)
addpath('../')
if nargin < 1
    fs = 48000;
...
C3 = full(C(hop+1:ck*fftLen, (hop+1:ck*fftLen)) );
imagesc(log(abs(C2)))
imagesc(log(abs(C3)))
% pruned2 = C2(hop + 1 : hop + fftLen, hop + 1 : hop + fftLen);
tt = tril(C2);
R = cholesky(tt, effectiveLen, fftLen, hop);
tt2 = tril(C3);
R2 = cholesky(tt2, effectiveLen, fftLen, hop);
% R3 = R2(fftLen-hop+1:end-hop+1, fftLen-hop+1:end-hop+1);
% Rm = choleskyComputationMap(C2);
toc
...
padUp = ic / halfLen * hop + hop;
padDown = hop*(Nm) - padUp;
tPhiSel2 = [zeros(padUp, halfLen); tPhi0; zeros(padDown, halfLen)];
...
Phi_s3 = R' \ (R \ tPhiSel(fftLen+1:ck*fftLen, :)); % pseudo inverse
Phi_s4 = R2' \ (R2 \ tPhiSel2(hop+1:ck*fftLen, :)); % pseudo inverse
Phi_s4 = Phi_s4(fftLen-hop+1:end, :);
