function y = ltv_inverse2(S, coeff)
fftLen = coeff.fftLen;
hop = coeff.hop;
wndCorrectionWeightingLF = coeff.wndCorrectionWeightingLF;
wndCorrectionWeightingHF = coeff.wndCorrectionWeightingHF;
correctionMatrix = coeff.correctionMatrix;
prepad = coeff.prepad;
pospad = coeff.pospad;
reqSynthesisWnd = coeff.reqSynthesisWnd;
nframes = size(S, 2);
halfLen = fftLen / 2 + 1;
accFrame2 = zeros(fftLen, 1);
frameindex = 1 + (0 : nframes - 1) * hop;
y = zeros(nframes * hop, 1);
for i=1:nframes
    %% Inverse transform
    q_fft_frameinv = S(:, i) .* coeff.corrF;
    q_fft_frameinv(halfLen+1:fftLen,:) = conj(q_fft_frameinv(halfLen-1:-1:2,:));
    STime1 = ifft(q_fft_frameinv);
    getbackCorrectedToSpectrum1 = fft(STime1 .* coeff.correctionWnd);
    % Frequency weighted interpolation between original and windowed spectra
    q_fft_frameinv = q_fft_frameinv(1:halfLen,:) .* wndCorrectionWeightingLF + getbackCorrectedToSpectrum1(1 : halfLen) .* wndCorrectionWeightingHF;
    if ~isempty(correctionMatrix)
        ext = [conj(q_fft_frameinv(prepad + 1 : -1 : 2)); q_fft_frameinv; conj(q_fft_frameinv(halfLen - 1 : -1 : (halfLen - pospad + 1)))];
        q_fft_frameinv = correctionMatrix * ext;
    end
    q_fft_frameinv(1) = real(q_fft_frameinv(1));
    q_fft_frameinv(end) = real(q_fft_frameinv(end));
    % Reflect spectrum and conjugate
    q_fft_frameinv(halfLen+1:fftLen,:) = conj(q_fft_frameinv(halfLen-1:-1:2,:));
    yInvShifted = ifftshift(ifft(q_fft_frameinv)) .* coeff.synWnd;
    % Overlap add
    accFrame2 = accFrame2 + yInvShifted;
    myOut = accFrame2(1 : hop);
    accFrame2 = [accFrame2(hop + 1 : end); zeros(hop, 1)];
    y(frameindex(i):frameindex(i)+hop-1) = myOut;
end
end