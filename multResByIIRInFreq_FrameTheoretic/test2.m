...
sg.S = ext;
sg.target = target;
func3_ = @(x) corrSplitCplxMtxFastExtended2(x, paddedHalfLen, paddedFFTLen, prepad, pospad, hop, sg, [], synWnd2, fftLen, eachSide);
...
function y = corrSplitCplxMtxFastExtended2(weights, paddedHalfLen, paddedFFTLen, prepad, pospad, hop, sigs, gWeights, synWnd, fftLen, eachSide)
x = ADNode(reshape(weights, paddedHalfLen, (paddedHalfLen + prepad + pospad - 1) * 2));
xRe = x(:, 1 : (paddedHalfLen + prepad + pospad - 1));
xIm = x(:, (paddedHalfLen + prepad + pospad - 1) + 1 : (paddedHalfLen + prepad + pospad - 1) * 2);
q_fft_frameinv2Re = pagemtimes(xRe, real(sigs.S));
q_fft_frameinv2Im = pagemtimes(xIm, imag(sigs.S));
q_fft_frameinv2Im(1, :, :) = 0;
q_fft_frameinv2Im(end, :, :) = 0;
td = separate_irfft(q_fft_frameinv2Re, q_fft_frameinv2Im, paddedFFTLen);
if ~isempty(synWnd)
    mtx = circshiftCustom(td, paddedFFTLen / 2) .* synWnd;
else
    mtx = circshiftCustom(td, paddedFFTLen / 2);
end
y3 = fold3D(mtx, paddedFFTLen, hop);
y = y3.value(fftLen - hop + eachSide + 1 : end, :);
end
