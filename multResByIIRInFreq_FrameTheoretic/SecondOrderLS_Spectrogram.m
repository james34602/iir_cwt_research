function [specgram, f, frmIdx, coeff] = SecondOrderLS_Spectrogram(x, fftLen, hop, fs, oct, order, reqSynthesisWnd)
rng(1)
...
%%
f = (0:1:fftLen/2)*fs/fftLen;
[specgram, frmIdx, front, back] = ltv_spectrogram2(x, coeff, outterPad);
end
...
function [S, frmIdx, front, back] = ltv_spectrogram2(x, coeff, outterPad)
x = x(:);
xLen = length(x);
...
for i=1:nframes
    ...
    q_fft_frameinv2 = q_fft_frameinv(1 : coeff.origHalfLen);
