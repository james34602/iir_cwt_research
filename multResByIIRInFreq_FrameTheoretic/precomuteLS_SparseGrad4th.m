function [coeff, f] = precomuteLS_SparseGrad4th(fftLen, hop, fs, oct, order, reqSynthesisWnd, sparCutOff, zp)
rng(1)
addpath('../')
...
outterPad = 0;
%%
specgram = ltv_spectrogram1(x, coeff, outterPad);
...
for m = 1 : nframes % index of window position
    ...
    q_fft_frameinv = ltv_1st_2ndNoSq1(Xa, Bmfwd, Bmbwd, Am, divareaSum);
...
function S = ltv_spectrogram1(x, coeff, outterPad)
x = x(:);
x = [zeros(outterPad + coeff.origFFTLen - coeff.hop, 1); x; zeros(outterPad + coeff.origFFTLen / 2, 1)];
ny = length(x);
nframes = fix(ceil(ny/coeff.hop) - coeff.origFFTLen / coeff.hop / 2);
