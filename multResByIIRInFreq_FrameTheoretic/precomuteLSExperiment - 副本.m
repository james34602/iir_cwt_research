function [coeff, f] = precomuteLSExperiment(fftLen, hop, fs, oct, order, reqSynthesisWnd, sparCutOff, zp)
rng(1)
addpath('../')
...
sigLen = 32768;
x = randn(sigLen, 1);
x = [zeros(size(Phi_s2, 1) * 0, 1); x(:); zeros(size(Phi_s2, 1), 1)];
...
shtfft1 = -(2*rem(0:(origHalfLen-1), 2) - 1)';
...
for m = 1 : nframes % index of window position
    ...
    q_fft_frameinv = shtfft2 .* q_fft_frameinv(1 : halfLen);
    q_fft_frameinv2 = q_fft_frameinv(1 : origHalfLen);
    q_fft_frameinv3 = q_fft_frameinv2;
    q_fft_frameinv2(origHalfLen+1:origFFTLen) = conj(q_fft_frameinv2(origHalfLen-1:-1:2));
    % plot(abs(Xa2 - q_fft_frameinv2))
    % axis tight
    % disp('')
    % td = [zeros(eachSide, 1); ifft(q_fft_frameinv); zeros(eachSide, 1)];
    % fd = fft(td);
    % met3 = sol4 * q_fft_frameinv3;
    met1 = sol3 * q_fft_frameinv2;
    met1(halfLen2+1:size(Phi_s2, 1)) = conj(met1(halfLen2-1:-1:2));
    met2 = sol2 * q_fft_frameinv2;
    plot(abs(met1-met2))
    % hold on
    % plot(abs(met2))
    % hold off
    axis tight
    WinvdftRe = real(Phi_s2);
    WinvdftIm = imag(Phi_s2); WinvdftIm(:, halfLen2 + 1 : end) = -WinvdftIm(:, halfLen2 + 1: end);
    q_fft_frameinv2 = q_fft_frameinv;
    q_fft_frameinv2(halfLen/2+1+1:halfLen) = conj(q_fft_frameinv2(halfLen/2+1-1:-1:2));
    td = WinvdftRe * real(q_fft_frameinv2) - WinvdftIm * imag(q_fft_frameinv2);
    % plot(abs(met1 - met2))
    % td1 = ifft(met1);
    % td2 = ifft(met2);
    % bb = gg * q_fft_frameinv;
    % figure(1)
    % plot(abs(q_fft_frameinv));
    % axis tight
    % figure(2)
    % plot(abs(bb));
    % axis tight
    % td = ifft(bb);
    % plot(real(td))
    ym = Phi_s2*q_fft_frameinv2; % reconstruct a block of Ls samples
