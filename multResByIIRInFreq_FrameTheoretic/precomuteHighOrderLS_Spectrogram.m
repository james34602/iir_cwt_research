function [dftspecgram, specgram, specgram4th, f, frmIdx, front, back] = precomuteHighOrderLS_Spectrogram(fftLen, hop, fs, oct, order, reqSynthesisWnd)
rng(1)
addpath('../')
...
return
s_q4th = abs(specgram4th) .^ 2;
s_q4th = s_q4th ./ max(abs(s_q4th(:)));
s_q = abs(specgram) .^ 2;
s_q = s_q ./ max(abs(s_q(:)));
...
function [S, Sdft, frmIdx, front, back] = ltv_spectrogram1(x, coeff, outterPad)
x = x(:);
xLen = length(x);
x = [zeros(outterPad + coeff.origFFTLen - coeff.hop, 1); x; zeros(outterPad + coeff.origFFTLen / 2, 1)];
front = outterPad + coeff.origFFTLen - coeff.hop;
