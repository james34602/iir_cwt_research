...
theoreticalWindowShape3 = zeros(size(theoreticalWindowShape1));
...
% Xa3 = Xa3(1 : halfLen, :);
b = b(prepad + 1 : end - pospad + 1);
c1 = c1(prepad + 1 : end - pospad + 1);
c2 = c2(prepad + 1 : end - pospad + 1);
b(halfLen+1:fftLen,:) = conj(b(halfLen-1:-1:2,:));
c1(halfLen+1:fftLen,:) = conj(c1(halfLen-1:-1:2,:));
c2(halfLen+1:fftLen,:) = conj(c2(halfLen-1:-1:2,:));
plot(del2(b));
hold on;
...
for idx = 1 : size(theoreticalWindowShape1, 1)
    ...
    % hold off
    % axis tight
    theoreticalWindowShape3(idx, :) = b2;
end
theoreticalWindowShape3 = (theoreticalWindowShape3) / fftLen;
theoreticalWindowShape3(halfLen+1:fftLen,:) = conj(theoreticalWindowShape3(halfLen-1:-1:2,:));
centred2 = theoreticalWindowShape3;
...
for m = 1 : nframes % index of window position
    ...
    % q_fft_frameinv = q_fft_frameinv(prepad + 1 : end - pospad + 1);
