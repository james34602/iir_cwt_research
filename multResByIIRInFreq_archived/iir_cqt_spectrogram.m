function [S, f, t] = iir_cqt_spectrogram(x,fftLen,hop,fs,Q)
...
x = x(:);
x = [zeros(fftLen, 1); x; zeros(fftLen / 2, 1)];
ny = length(x);
...
slope = pi/fftLen;
thetas = abs(slope*linspace(-prepad,fftLen/2+pospad-1,fftLen/2+prepad+pospad));
thetas(prepad+1) = eps; % zero digital frequency
% poles of the IIR LTV Q FFT transform for the parameters above
poles = calculate_poles(thetas,Q,fftLen);

% IIR LTV Q FFT transform
for i=1:nframes
    S(i,:) = iir_ltv_q_fft(x(frameindex(i):frameindex(i)+fftLen-1)',poles, prepad, pospad);
end

t = (frameindex-1+fftLen/2)/fs;
f = (0:1:fftLen/2-1)*fs/fftLen;
S = S';
