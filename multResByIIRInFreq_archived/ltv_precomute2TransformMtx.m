function [coeff, f] = ltv_precomute2TransformMtx(fftLen, hop, fs, oct, order, HFSamplingLimit, gaussSigmaLimiting, cornerSmoothing, reqSynthesisWnd)
...
%     err = abs(fft(err));
%     err = err(1 : halfLen);
%     time = zp * 0.048 * fftLen;
%     alpha = 1 / (1 + time);
%     [bSm2, aSm2] = butter(1, alpha);
%     sm = filtfilt(bSm2, aSm2, err);
%     sm = sm ./ max(sm);
%     [~, idx] = sort(sm);
    % Take the frequency from max power
%     transitionBin = idx(int32(halfLen));
%     correctionWndTransition = overlapAdd(repmat(theoreticalWindowShape(transitionBin, :), ovp * 2, 1 )', hop);
%     correctionWndTransition = correctionWndTransition(fftLen - hop + 1 : fftLen * 2 - hop);
%     correctionWndTransition = 1 ./ correctionWndTransition(:);
    ...
    for j = 1:halfLen
        gTime = theoreticalWindowShape(j, :);
        ...
        g2 = g2(1 : halfLen);
        transformationMatrix(:, j) = g2;
    end
    %%
    transformationMatrix(halfLen+1:fftLen,:) = conj(transformationMatrix(halfLen-1:-1:2,:));
    transformationMatrix(:, halfLen+1:fftLen) = transformationMatrix(:, halfLen-1:-1:2);
    normalizationVector = ones(fftLen, 1) / fftLen;
    weights1 = transformationMatrix;
    getbackCorrectedToSpectrum1 = [];
    getbackCorrectedToSpectrum2 = [];
    getbackCorrectedToSpectrum3 = [];
    func1 = @(x) singleWndSplitMtx(x, halfLen, fftLen, hop, normalizationVector, S1, getbackCorrectedToSpectrum1, bandlimitChirp, S2, getbackCorrectedToSpectrum2, shiftedChirp1, S3, getbackCorrectedToSpectrum3, bandlimitChirp2);
    [mse1, gd1] = func1(weights1);
    opt.optTol = 1e-14;
    opt.progTol = 1e-14;
...
function [f, g] = singleWndSplitMtx(weights, halfLen, frameSize, hop, normalizationVector, S1, getbackCorrectedToSpectrum1, bandlimitChirp, S2, getbackCorrectedToSpectrum2, shiftedChirp1, S3, getbackCorrectedToSpectrum3, shiftedChirp2)
corrMtx = ADNode(weights);
%% Inverse transform
% Frequency weighted interpolation between original and windowed spectra
q_fft_frameinv2 = corrMtx * S1;
...
meaSqr = abs(y - bandlimitChirp) .^ 2;
mse1 = sum(meaSqr .* normalizationVector);
%% Inverse transform
% Frequency weighted interpolation between original and windowed spectra
q_fft_frameinv2 = corrMtx * S2;
% q_fft_frameinv2(halfLen+1:fftLen,:) = conj(q_fft_frameinv2(halfLen-1:-1:2,:));
mtx = circshift(ifft(q_fft_frameinv2), frameSize / 2);
y3 = fold(mtx, frameSize, hop);
y = y3(frameSize - hop + 1 : frameSize - hop + frameSize);
meaSqr = abs(y - shiftedChirp1) .^ 2;
mse2 = sum(meaSqr .* normalizationVector);
%% Inverse transform
% Frequency weighted interpolation between original and windowed spectra
q_fft_frameinv2 = corrMtx * S3;
% q_fft_frameinv2(halfLen+1:fftLen,:) = conj(q_fft_frameinv2(halfLen-1:-1:2,:));
mtx = circshift(ifft(q_fft_frameinv2), frameSize / 2);
y3 = fold(mtx, frameSize, hop);
y = y3(frameSize - hop + 1 : frameSize - hop + frameSize);
meaSqr = abs(y - shiftedChirp2) .^ 2;
mse3 = sum(meaSqr .* normalizationVector);
mse = (mse1 + mse2 + mse3) / 3;
f = mse.value;
g = mse.backprop(1);
end
function [f, g] = singleWndSplitMtxBackProp(weights, halfLen, frameSize, hop, normalizationVector, S1, getbackCorrectedToSpectrum1, bandlimitChirp, S2, getbackCorrectedToSpectrum2, shiftedChirp1, S3, getbackCorrectedToSpectrum3, shiftedChirp2)
corrMtx = ADNode(weights);
%% Inverse transform
% Frequency weighted interpolation between original and windowed spectra
q_fft_frameinv2 = corrMtx * S1;
