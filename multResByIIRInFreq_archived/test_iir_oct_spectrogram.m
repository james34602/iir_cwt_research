...
[coeff, f_q] = ltv_precomute2(fftLen, hop, fs, oct, order, HFSamplingLimit, 1, 1, reqSynthesisWnd);
% [coeff, f_q] = ltv_precomute(fftLen, hop, fs, oct, order, HFSamplingLimit, 1, 1, reqSynthesisWnd);
...
%     editedBk = editing;
    if isfield(coeff, 'wndCorrectionWeightingLF')
        y = ltv_inverse2(editing, fftLen, hop, reqSynthesisWnd, coeff.correctionWnd, coeff.wndCorrectionWeightingLF, coeff.wndCorrectionWeightingHF);
    else
        y = ltv_inverse(editing, fftLen, hop, reqSynthesisWnd, coeff.correctionWnd, coeff.wndCorrectionWeighting);
    end
    y = y(:);
