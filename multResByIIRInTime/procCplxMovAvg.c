...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    if (nrhs != 2)
        mexErrMsgIdAndTxt("MATLAB:procCplxMovAvg:invalidNumInputs", "2 input arguments are required.");
    else if (nlhs > 1)
        mexErrMsgIdAndTxt("MATLAB:procCplxMovAvg:maxlhs", "Too many output arguments.");
    ...
    if (!mxIsDouble(prhs[1]) || mxIsSparse(prhs[1]) || (MIN(m, n) != 1))
        mexErrMsgIdAndTxt("MATLAB:procCplxMovAvg:incorrectType", "Array must be double vector");
    ...
    if(!mxIsComplex(prhs[1]))
    {
        ...
        procCplxMovAvgReal(yp, x, y);
    }
    else
    {
        mxComplexDouble *x = mxGetComplexDoubles(prhs[1]);
        procCplxMovAvgCplx(yp, x, y);
    }
