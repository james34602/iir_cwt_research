% x=zeros(4096, 1);
% x(2048)=1;
% plot(Deriche1D(x, 256))
function [y, b1, a1, b2, a2] = Deriche1D(x,fsigma)
[npixels2, npixels] = size(x);
if (1 ~= npixels2 && 1 ~= npixels)
    fprintf('\n\n');
    fprintf('     Input ERROR! Input  x must be a vector. \n');
    fprintf('     -----------------------------------------------  ');
    fprintf('\n\n');
    return
end
